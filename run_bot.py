...
def write_msg_attach(user_id, text, att_url):
    vk_bot.method('messages.send',
                  {'user_id': user_id, 'attachment': att_url, 'message': text, 'random_id': random.randint(0, 5000)})
    ...
    if update[0][0] == 4:
        ...
        if 'прив' in update[0][6]:
            write_msg(user_id, 'привет, ' + (
                        user_name[0]['first_name'] + '.' + ' ' + 'Что бы узнать что я могу, введи "команды".'))
